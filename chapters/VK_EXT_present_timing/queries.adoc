...
include::{generated}/api/structs/VkPastPresentationTimingEXT.adoc[]
  ...
  * pname:targetPresentTime is an application-provided value that was given
    ...
    pname:actualPresentTime results of consecutive presents.
  * pname:actualPresentTime is the time when the image of the
    pname:swapchain was actually displayed, when the first pixel of
    the image was made visible to the user. It may: be zero if the
    implementation was unable to determine an accurate time, or if the
    presentation engine discarded the present request.
  * pname:actualPresentCutoffTime is the time before which the image of the
    pname:swapchain needed to be ready to be eligible to be presented at
    pname:actualPresentTime. It is zero if pname:actualPresentTime is zero.
  ...
  * pname:timeDomainChanged is ename:VK_TRUE if the time domain enabled for
    ...
    enable present timing with a currently-available time domain.
...
if a lower IPD could be durably achieved by computing the difference between
